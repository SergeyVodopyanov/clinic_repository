...
class CreateRecords3Table extends Migration
{
    ...
    public function up()
    {
        Schema::create('records3', function (Blueprint $table) {
            ...
            $table->foreign('patient_id', 'record3_patient_fk')->on('patients')->references('id');

            $table->unsignedBigInteger('speciality_id')->nullable();
            $table->index('speciality_id', 'record3_speciality_idx');
            $table->foreign('speciality_id', 'record3_speciality_fk')->on('specialities')->references('id');

            $table->unsignedBigInteger('doctor_id')->nullable();
            $table->index('doctor_id', 'record3_doctor_idx');
            $table->foreign('doctor_id', 'record3_doctor_fk')->on('doctors')->references('id');

            $table->unsignedBigInteger('service_id')->nullable();
            $table->index('service_id', 'record3_service_idx');
            $table->foreign('service_id', 'record3_service_fk')->on('services')->references('id');
